---
description: Adoptar BDD (Behavior-Driven Development) en el proyecto - setup, Three Amigos, primer .feature
---
...
Lee `@prompts/skills/bdd-skill.md` y pregunta al usuario:
...
Si el usuario responde NO o duda, **NO procedas**. Cierra el comando con una recomendacion: "Mantener tests-skill.md y visual-testing-skill.md como cobertura. Volver a /bdd cuando aparezcan los stakeholders no tecnicos."
...
Aplica la **regla de eleccion en 2026** de `bdd-skill.md`:
...
Una vez confirmada la herramienta, instala y configura siguiendo `bdd-skill.md`:
...
Crea/actualiza `playwright.config.ts` con `defineBddConfig`. Si ya existe un `playwright.config.ts` del setup E2E previo, **NO lo sobreescribas**: anade la configuracion BDD coexistiendo (ver `bdd-skill.md` para el patron).
...
Si el usuario procede, ejecuta el sub-protocolo de Example Mapping (ver `bdd-skill.md` seccion "Three Amigos y Example Mapping"):
...
1. Genera los step definitions en `features/steps/` siguiendo el patron de `bdd-skill.md`.
