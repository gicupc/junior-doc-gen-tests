...
Es transversal a `tests-skill.md`, `integration-testing-skill.md`, `bdd-skill.md` y `visual-testing-skill.md`: aplica a todas ellas cuando se introduce IA en el flujo.
...
2. El agente abre el navegador vía Playwright MCP.
3. El agente explora la UI real, identifica elementos por rol/etiqueta.
4. El agente escribe el spec en TypeScript.
5. El agente lo ejecuta y lo ajusta hasta que pasa de forma estable (3 veces sin flake).
...
**Recomendación oficial de Microsoft (2026)**: usar CLI cuando el agente tiene filesystem, MCP cuando el cliente es chat puro.
...
Lo que separa un test generado por IA estable de uno frágil suele ser el prompt. Patrones que funcionan (siguiendo `prompt-skill.md` RACEO):
...
- **`On(testing_setup)`** menciona esta skill si el usuario indica que va a usar agentes IA para tests. Carga `ai-testing-skill.md` como guía adicional.
