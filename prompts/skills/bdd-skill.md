...
Prompt efectivo (siguiendo el patrón RACEO de `prompt-skill.md`):
...
- **`On(bdd_setup)`** (definido en `architect-brain.md`) se ejecuta cuando el proyecto adopta BDD durante `/inicio` o `/regularizar`. Instala `playwright-bdd` (o `@cucumber/cucumber` si no hay UI), crea estructura `features/` y `features/steps/`, añade scripts a `package.json`, registra ADR, añade tickets al `roadmap.md`.
