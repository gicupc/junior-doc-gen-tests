...
| 2. Componente | ¿Este componente renderiza/reacciona correctamente y es accesible? | Vitest/Jest + RTL + jest-axe | Rápido |
...
Stack: **Vitest + React Testing Library (RTL) + jest-axe** (la versión `vitest-axe` para Vitest puro).
...
- Buscar elementos por **rol** (`getByRole`) o **etiqueta accesible** (`getByLabelText`), nunca por clase ni testid si hay alternativa accesible. Si tu componente solo se puede testear por testid, probablemente no es accesible.
...
- Si axe reporta violaciones, NO suprimir la regla salvo justificación documentada (algunas reglas dan falsos positivos en componentes aislados que se vuelven correctos en contexto).
...
- Playwright opera sobre el accessibility tree (determinista, sin flake por races).
...
Para diseños con alta exigencia de fidelidad visual (rebrand, design system reutilizable):
...
1. La primera vez que se ejecuta, se crea el baseline a partir del diseño aprobado (capturando con el código actual).
2. En sucesivas ejecuciones, el test pasa si la diferencia es <0.5% (umbral configurable).
3. Cuando un cambio intencional rompe el baseline, se regenera con `rm tests/visual/baselines/* && npm run test:visual` (solo tras revisión humana).
4. **Patrón con agente**: agente genera código → Playwright corre → si rojo, agente lee el `diff.png` con su modelo de visión, ajusta código, vuelve a ejecutar. Cierre del loop sin intervención humana en pasos triviales.
...
tests/
  ...
  unit/                             # Lógica pura (función, hook, util)
    validators.test.ts
...
Regla operativa: **el test cuesta menos que el bug que evita**. Si dudas, escribe el test.
...
- **`On(task_complete)`** debe verificar que toda página/componente nuevo tiene su test de a11y. Si falta, no cerrar la tarea.
...
- **`/revisar-frontend`** ejecuta auditoría de a11y como una de las 4 categorías. Lee los reports de los tests si existen.

---

## Antipatrones a evitar

- **Tests de componente que buscan por clase CSS**: `container.querySelector('.btn-primary')`. Si tu test no usa rol o label accesible, probablemente el componente no es accesible. Refactor.
...
- **Snapshot tests gigantes**: hacer `toMatchSnapshot()` de un componente de 200 nodos hace que cualquier cambio rompa el test. Mejor: assertions específicas (`toHaveTextContent`, `toHaveAttribute`).
- **Tests E2E que dependen de orden**: si `test_b` necesita que `test_a` haya creado un usuario, los tests no son independientes. Cada test crea su propio fixture.
- **Visual diff con umbral 0%**: nunca pasa por sub-pixel rendering. Mínimo 0.1% de tolerancia.
- **Tests de a11y solo automatizados**: tooling detecta ~30% de issues. Para feature crítica, validación manual con NVDA/VoiceOver al menos una vez antes del release.
- **Playwright lanzado contra producción real**: nunca. Usa entornos staging o `webServer` local. Riesgo de generar datos basura en producción.
