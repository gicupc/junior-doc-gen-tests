...
Esta skill enseña al **usuario** cómo redactar prompts de calidad cuando interactúa con Cascade durante el ciclo de un proyecto. No la consume Cascade automáticamente: es un manual operativo para la persona que escribe.

Se usa especialmente durante:
- Onboarding a un codebase desconocido (`/onboarding`).
- Planificación de features complejas (`/nueva`).
- Implementación paso a paso (cuando el usuario reparte trabajo a Cascade).
...
```
# Role          — quién debe ser Cascade en esta tarea
...
# Constraints   — qué NO debe hacer Cascade
...
### Role
La diferencia entre "explícame este código" y "Eres un senior haciendo onboarding rápido a un codebase legacy" es enorme. El rol moldea tono, profundidad y prioridades. Sin rol, Cascade tiende a sobre-explicar lo trivial e infra-explicar lo importante.
...
### Context
Lo que Cascade necesita saber pero que no está obvio en el código: stack, restricciones del entorno, decisiones previas tomadas, qué partes del código son intocables. **Si en el chat anterior ya quedó claro algo, no lo repitas** — Cascade conserva el contexto del turno previo.

### Tasks
Cuando el prompt requiere múltiples acciones, numéralas. La numeración fuerza orden de ejecución y permite parar a Cascade entre pasos.
...
### Expected Output
Define el formato. "Markdown con 5 secciones, máximo 15 líneas cada una" es mejor que "explícamelo bien". Si no defines formato, Cascade improvisa.
...
```markdown
# Role
[Quién es Cascade en esta tarea — un rol concreto, no genérico]
...
1. **"Hazlo bien"** — sin role, sin formato esperado, sin restricciones. Cascade improvisa.
2. **Pedir 5 cosas en una frase** — mejor 5 prompts cortos que uno largo. Cascade gestiona mejor un foco a la vez.
3. **No detener entre pasos en implementacion** — si pides "implementa A, B y C", Cascade lo hace todo de un tiron y pierdes la oportunidad de revisar B antes de C.
4. **Repetir contexto que ya esta en el chat** — Cascade conserva el turno anterior. No le repitas el README cada vez.
