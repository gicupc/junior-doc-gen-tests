# Architect-Brain v4.7.1 (Template Detection Refined + Supply Chain Security + Testing 2026 BDD + Integration + AI-Assisted + Frontend + MCP + Database)
...
5. **Si el proyecto tiene frontend** y nivel T es 2 o 3, anade tambien:
   ...
   - Carga `@prompts/skills/visual-testing-skill.md` como guia.
6. **Si el proyecto tiene APIs HTTP o BD** y nivel T es 2 o 3, anade tambien:
   ...
   - Carga `@prompts/skills/integration-testing-skill.md` como guia.
...
11. **Pregunta al usuario sobre uso de IA en testing**:
    - Si va a usar Cursor, Claude Code, Copilot u otro agente para generar tests, carga `@prompts/skills/ai-testing-skill.md` como guia adicional.
...
12. **Si el proyecto usa npm registry** (Node.js, JS/TS, cualquier stack con `package.json`), ejecuta `On(supply_chain_setup)`:
    ...
    - Carga `@prompts/skills/supply-chain-skill.md` como guia obligatoria.
...
Ver detalle completo en `.windsurf/workflows/bdd.md`.

Carga `@prompts/skills/bdd-skill.md` como guia obligatoria.
...
Ver detalle completo en `.windsurf/workflows/revisar-supply-chain.md`.

Resumen del flujo:
1. Verificar aplicabilidad: existe `package.json`. Si no, salir.
2. Cargar `@prompts/skills/supply-chain-skill.md` como guia.
...
9. Si detecta paquete comprometido instalado: protocolo de respuesta a incidente (NO revocar token de GitHub antes de limpiar persistencia, ver `supply-chain-skill.md`).
10. Cierre con `On(task_complete)`.

---

## Protocolo On(database_setup): [v4.2]
Se ejecuta tras `On(testing_setup)` cuando el proyecto incluye base de datos.

Carga `@prompts/skills/database-skill.md` como guia obligatoria.
...
Carga `@prompts/skills/frontend-skill.md` como guia obligatoria.
Carga ADICIONALMENTE `@prompts/skills/visual-testing-skill.md` cuando haya que setupear tests de UI.
...
2. **Inicializa configuracion**:
   ...
   - Si shadcn: pregunta al usuario antes de ejecutar `npx shadcn@latest init` (lo deja en `.cursor/`-like setup o componente local segun stack).
3. **Pregunta al usuario sobre el flujo diseno-a-codigo**:
   ...
   - Segun la respuesta, propon MCP servers a activar:
     - Figma con Dev Mode → recomienda activar **Figma Dev Mode MCP server** y, si shadcn, **shadcn MCP**.
     - Solo capturas → recomienda **Chrome DevTools MCP** y **Playwright MCP** para validacion visual.
     - Sin diseno → solo **shadcn MCP** para acceso a bloques.
   - Documenta los MCP servers elegidos en `docs/frontend-strategy.md` y, si la version de la estructura los soporta, en `.cursor/mcp.json` (template) o instruye al usuario a configurarlos en Windsurf MCP global.
...
Ver detalle completo en `.windsurf/workflows/revisar-frontend.md`.

Resumen del flujo:
1. Determinar alcance de la auditoria (completa o parcial: stack, seguridad, rendimiento, accesibilidad).
2. Cargar `@prompts/skills/frontend-skill.md` como guia.
...
6. Generar reporte ejecutivo en `docs/frontend-audit.md` y compartir hallazgos de seguridad con `docs/investigacion_seguridad.md` (que sirve tanto a BD como a frontend).
...
Esta fase exige disciplina explicita para evitar que Cascade encadene varios archivos sin oportunidad de revision.
...
7. **El paso final es siempre tests + ejecucion de la suite**. Pega el output completo de la ejecucion. Si hay rojos, NO continues hasta que el usuario decida.
...
Si un item NO aplica, marca `[x]` con la razon "sin cambios" en vez de saltarlo. La idea es que el usuario VEA que se considero cada doc.
