# Changelog v4.5 → v4.6 (Supply Chain Security Edition)
...
### 1. Skill nueva: `prompts/skills/supply-chain-skill.md`

**Estado:** archivo nuevo (~20 KB).

Cubre la cadena de suministro end-to-end:
...
- **Capa 1 detallada**: setup completo por package manager. Bloque pnpm (`pnpm-workspace.yaml`), bloque npm 11.10+ (`.npmrc`), bloque Yarn Berry 4.10+ (`.yarnrc.yml`), bloque Bun 1.3+ (`bunfig.toml`), bloque Renovate (`renovate.json`) y Dependabot (`.github/dependabot.yml`). Distinción entre `minimumReleaseAge` (todos los upgrades) y security updates (fast-track en Dependabot).
- **Capa 2 detallada**: política "deny by default" con `allowBuilds` (pnpm) o `--ignore-scripts` (npm), comandos para identificar paquetes con build scripts (`pnpm install --dry-run`, `npm explain`), defensa contra lockfile injection con `--frozen-lockfile` en CI, `enableHardenedMode` de Yarn Berry.
- **Capa 3 detallada**: el patrón "Pwn Request" como antipatrón NUNCA usar, patrones seguros con `pull_request_target` solo para operaciones de lectura (etiquetas, comentarios), mitigación de cache poisoning, pineado de acciones a SHA en lugar de tag (caso Trivy: 76 tags repointed), permisos mínimos por job.
- **Capa 4 detallada**: tabla comparativa PAT clásico vs OIDC, configuración de trusted publisher en npmjs.com con pinning a `Repository + Workflow + Branch` (NO solo Workflow), branch protection con "Block force pushes" (incluido en el ataque TanStack), 2FA obligatorio (TOTP/hardware key, no SMS).
- **Respuesta a incidente con orden CRÍTICO**: 6 pasos en orden inviolable.
  1. NO revocar token de GitHub todavía (vector `rm -rf ~/` al revocar).
  2. Limpiar daemon de persistencia (`gh-token-monitor` en `~/Library/LaunchAgents/` macOS, `~/.config/systemd/user/` Linux, Task Scheduler Windows).
  3. Buscar payloads residuales (`router_runtime.js`, `setup.mjs`, `tanstack_runner.js`) en `~/.claude`, `~/.vscode`.
  4. Aislar la máquina de la red.
  5. Rotar TODAS las credenciales accesibles desde la máquina.
  6. Auditar blast radius y documentar ADR.
- **Integración con harness**: cómo conecta con `On(testing_setup)`, `On(supply_chain_setup)`, `On(supply_chain_audit)`, `/revisar-supply-chain`, `On(task_complete)`, `/sync`.
- **Checklist obligatorio** antes de mergear cambios en dependencias (6 puntos).
- **Antipatrones operativos** (9 entradas) y regla de oro final.

### 2. Workflow nuevo: `.windsurf/workflows/revisar-supply-chain.md`

**Estado:** archivo nuevo (~9 KB).

Disparadores: "revisa supply chain", "audita las dependencias", "¿mi npm install es seguro?", "¿me afectó lo de TanStack?". Pasos:

1. Verificar aplicabilidad (existe `package.json`).
2. Cargar la skill `supply-chain-skill.md`.
...
- Versión `v4.5` → `v4.6 (Supply Chain Security Edition + Testing 2026 BDD + Integration + AI-Assisted + Frontend + MCP + Database)`.
...
- Insertado protocolo NUEVO `On(supply_chain_setup)` antes de `On(database_setup)` (10 puntos): detectar package manager → aplicar defensas Capa 1 (`minimumReleaseAge` adaptado al PM) → aplicar Capa 2 (`allowBuilds` / `ignore-scripts`) → auditar workflows existentes sin modificar (NO `pull_request_target` con checkout de fork, acciones sin SHA) → recordar OIDC con `workflow + branch` pinning si publica al registro → configurar Renovate/Dependabot con cooldown → generar `supply-chain-strategy.md` → registrar ADR → tickets de roadmap → cierre con `On(task_complete)`.
- Insertado protocolo NUEVO `On(supply_chain_audit)` después de `On(supply_chain_setup)` (10 puntos): resume el workflow `/revisar-supply-chain` con orden de respuesta a incidente.
- `On(task_complete)` checklist: añadidas líneas `supply-chain-strategy.md` y "Defensas supply chain intactas".
- `On(sync_check)`: añadidas 5 divergencias supply chain (`minimumReleaseAge` desactivado sin ADR, `pull_request_target` + checkout de fork, lockfile sin commitear o sospechoso, `allowBuilds` ampliado sin justificación, acciones de SHA a tag mutable).
...
- "Restricciones y Calidad": añadida consulta a `docs/supply-chain-strategy.md` antes de tocar dependencias, ejecución de `On(supply_chain_audit)` si encuentra proyecto JS/TS sin defensas, y 4 reglas operativas (NUNCA desactivar `minimumReleaseAge` sin ADR, NUNCA `pull_request_target` con checkout de fork, orden crítico de respuesta a incidente, defensa en 4 capas).
...
- CASO A (proyecto nuevo): añadido paso 7 que dispara `On(supply_chain_setup)` si el proyecto usa npm registry, con detalles de qué configura.
...
- Tabla de comandos: añadida fila para `/revisar-supply-chain` con disparadores ("revisa supply chain", "audita las dependencias", "¿mi npm install es seguro?", "¿me afecto lo de TanStack?").
- Bloque NUEVO "SUPPLY CHAIN (obligatorio si el proyecto usa npm registry)" después del bloque FRONTEND: lista las 4 capas, defaults por package manager, reglas operativas (lockfile commiteado, `npm ci` en CI), orden de respuesta a incidente, carga obligatoria de `supply-chain-skill.md`.
...
Añadida sección "Supply chain (si el proyecto usa npm registry)" al final con: configuración del package manager (ejemplo pnpm 11), tests con dependencias muy nuevas (NO en CI sin excepción documentada), CI hardening (`--frozen-lockfile`, no `pull_request_target` + fork), respuesta a incidente (orden crítico).

### 7. Modificado: `CLAUDE.md`

- Versión `v4.5` → `v4.6`. Mención explícita a cadena de suministro.
- Lista de protocolos `On(...)` actualizada (añadidos `bdd_setup`, `supply_chain_setup`, `supply_chain_audit`).
- SSOT amplía con `supply-chain-strategy.md`.
- Slash commands: añadido `/revisar-supply-chain`.
- Skills auxiliares: añadida `supply-chain-skill.md`.
...
- Slash commands: añadido `/revisar-supply-chain`.
- Skills auxiliares: añadida `supply-chain-skill.md`.
- Reglas de oro 14 y 15 nuevas (idem CLAUDE.md).
- SSOT amplía con `supply-chain-strategy.md`.

### 9. Modificado: `README.md`

- Cabecera amplía descripción con "cadena de suministro (npm + GitHub Actions + OIDC publishing)".
- Sección "Qué resuelve": 7 → 8 problemas (punto 8 nuevo sobre `npm install` y ataques tipo Shai-Hulud/TanStack).
- Sección "Slash commands disponibles": 13 → 14 commands; fila nueva `/revisar-supply-chain`.
- Sección "Qué hace el sistema": 9 → 10 pilares; pilar 10 nuevo "Defensa de cadena de suministro en 4 capas" con explicación detallada y mención a respuesta a incidente.
- Estructura de archivos: workflows 13 → 14 (incluido `revisar-supply-chain.md`), docs con `supply-chain-strategy.md`, skills 9 → 10 (incluida `supply-chain-skill.md`).
...
- Cabecera versión y subtítulo actualizados a v4.6.
...
- Reglas de oro: 17 → 20 con entradas 18, 19 y 20 nuevas.
- Estructura de archivos: workflows 13 → 14, docs con `supply-chain-strategy.md`, skills 9 → 10.
...
- `prompts/skills/supply-chain-skill.md`
- `.windsurf/workflows/revisar-supply-chain.md`
...
1. **Slash commands**: en Cascade, escribir `/` y verificar que aparece `/revisar-supply-chain` en la lista (total 14 comandos).
2. **Carga de skill**: invocar `@prompts/skills/supply-chain-skill.md` y comprobar que se carga sin errores.
...
5. **Orden crítico de respuesta a incidente**: preguntar "¿qué hago si he instalado un paquete comprometido?" y verificar que la IA responde con "NO revocar token de GitHub antes de limpiar persistencia" como primer paso, no como pie de página.
...
## Paridad con Cursor v4.6

Esta release tiene **paridad total** con la versión Cursor (`junior-doc-gen-tests-cursor`) v4.6, publicada en el mismo bump:

- Mismos archivos nuevos (4): skill, workflow (como skill en `.cursor/skills/`), doc SSOT, changelog.
...
- **Cursor**: skill en `.cursor/skills/X/SKILL.md` con frontmatter `name:` + `description:` extensa (sin tildes en `description`), reglas globales en `.cursor/rules/architect-brain.mdc`. Las referencias a otras skills usan nombres sin extensión (`supply-chain-skill`) en lugar de paths.
