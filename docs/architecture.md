# Arquitectura del Sistema (Architect-Brain v4.3) — Estándar Universal

> Meta-documentación del scaffolding `junior-doc-gen-tests`. Describe cómo está construido el propio sistema, no el proyecto que lo use. Para el SSOT del proyecto concreto, ver el resto de archivos en `/docs/`.
...
- **Interfaz:** Windsurf/Cascade operando bajo el protocolo definido en `@.windsurfrules`.
...
- **Single Source of Truth (SSOT):** Carpeta `/docs/` en la raíz del proyecto. Ninguna documentación oficial vive fuera de aquí. La memoria auto-generada de Windsurf NO es SSOT.
...
### Skills estáticas (vienen con el sistema)
Distribuidas en `@prompts/skills/` desde el primer día:

- **`tests-skill.md`** — convenciones de testing, patrones de mock por stack, antipatrones.
- **`legacy-testing-skill.md`** — protocolo de characterization tests para código legacy sin red de seguridad.
- **`database-skill.md`** — auditorías de modelo, seguridad, rendimiento y calidad de datos.
- **`prompt-skill.md`** *(nuevo en v4.3)* — manual del **usuario** para redactar prompts profesionales con el patrón **RACEO** (Role + Objective + Context + Constraints + Expected Output).

### Skills dinámicas (las genera la IA durante el proyecto)
Cuando aparece lógica compleja específica del dominio (reglas fiscales, integraciones SFTP, flujos de pago), Cascade crea archivos nuevos en `@prompts/skills/` para que el conocimiento persista entre sesiones.
...
- **[ONBOARDING]** *(nuevo en v4.3)* — mapa rápido de codebase ajeno sin generar SSOT (workflow `/onboarding`). Solo lectura.
...
- **Prompts profesionales:** el usuario consulta `prompt-skill.md` para tareas complejas; el patrón RACEO reduce alucinaciones.
