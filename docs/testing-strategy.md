...
Este proyecto sigue la siguiente distribución de niveles de testing. Cada nivel cubre preguntas diferentes y tiene su skill asociada en `prompts/skills/`.

| Nivel | Pregunta que responde | Skill | Cobertura objetivo |
|---|---|---|---|
| **Unit** | ¿Esta función/hook/clase se comporta correctamente con estos datos? | `tests-skill.md` | Toda la lógica crítica |
| **Integración** | ¿Las piezas reales colaboran correctamente (API + BD + servicios)? | `integration-testing-skill.md` | Endpoints HTTP, flujos servicio-repositorio, componentes con fetch real |
| **E2E + visual + a11y** | ¿El sistema completo funciona como un usuario lo espera? | `visual-testing-skill.md` | Flujos críticos del usuario (login, checkout, alta) |
| **BDD / Aceptación** *(opcional)* | ¿El comportamiento cumple los criterios acordados con negocio? | `bdd-skill.md` | Solo features acordadas vía Three Amigos |
| **Legacy / Characterization** *(condicional)* | ¿Qué hace HOY este código legacy antes de tocarlo? | `legacy-testing-skill.md` | Solo código legacy sospechoso |
...
- [ ] Anti-patrones de Gherkin generado por IA conocidos por el equipo (ver `bdd-skill.md`).
...
*Ejemplo:*
- **Generación de tests E2E**: Claude Code con Playwright MCP.
...
Documentación completa en `prompts/skills/ai-testing-skill.md`.
...
Referencia: `prompts/skills/ai-testing-skill.md` sección "Riesgos y límites".
...
Documentación completa en `prompts/skills/supply-chain-skill.md`.
